<#
    .SYNOPSIS
        Collects information from ADFS event 411
    .DESCRIPTION
        This script collects AD FS logon failed events (EventId 411) and
        reports them to pipeline or to .CSV file
    .EXAMPLE
        Get-AdfsAuditFailed.ps1 -After ([datetime]::Today) -Path report.csv

        Reports all AD FS failed logon events for today.  The result is saved to
        report.csv file
    .EXAMPLE
        Get-AdfsAuditFailed.ps1 -After 2021.05.01 -User user@domain.com
...
$events = Get-WinEvent -FilterHashtable $EventFilter |
    ForEach-Object {
        ...
        $eventProps = @{
            ...
            Id           = $currentEvent.Id
            User         = $currentEvent.Properties[2].Value.split('-')[0]
            IP           = $currentEvent.Properties[4].Value
            ErrorMessage = $currentEvent.Properties[2].Value
        }
